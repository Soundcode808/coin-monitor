...
## 13. Oracle Cloud 서버에서 24시간 실행하기
...
### 프로그램 상태 확인
```
sudo systemctl status coin-monitor
```

### 프로그램 로그 실시간 보기
```
sudo journalctl -u coin-monitor -f
```

### 프로그램 재시작
```
sudo systemctl restart coin-monitor
```

### 프로그램 중지 / 시작
```
sudo systemctl stop coin-monitor
sudo systemctl start coin-monitor
```
...
Claude Code에서 코드 수정 → 맥북 터미널에서 GitHub에 올리기:
```
cd "/Users/vesper/Documents/코인 모니터링"
git add .
git commit -m "수정 내용"
git push
```

Oracle 서버 터미널에서 받아오기:
```
cd coin-monitor && git pull && sudo systemctl restart coin-monitor
```
