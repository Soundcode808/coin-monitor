...
### 서버 정보
- **IP**: `192.18.137.207`
- **SSH 키 위치**: `~/Downloads/ssh-key-2026-04-20.key`
- **서버 내 코드 경로**: `~/coin-monitor`

---

### A. 맥 껐다 켠 후 처음 시작하는 순서
...
ps aux | grep main.py
```
→ 결과에 `python3 main.py`가 보이면 이미 실행 중 (아무것도 안 해도 됨)
→ 아무것도 없으면 3단계 진행
...
```
→ 에러 없이 `공통 코인 추출 완료`, `모니터링 시작` 로그가 보이면 정상
→ 확인 후 `Ctrl+C` 로 로그 보기 종료 (프로그램은 계속 돌아감)

---

### B. 코드 수정 후 서버 반영하는 순서

**1단계 — 맥북 터미널에서 GitHub에 올리기** (코인 모니터링 폴더에서):
```
cd "/Users/vesper/Documents/코인 모니터링" && git add exchanges.py main.py notifier.py fx.py && git commit -m "수정내용" && git push
```
...
**3단계 — 서버에서 받아오고 재실행:**
```
pkill -f "python3 main.py"; cd ~/coin-monitor && git checkout -- log.txt && git pull && nohup python3 main.py > log.txt 2>&1 &
```
...
| 작업 | 명령어 |
|------|--------|
| 프로그램 중지 | `pkill -f "python3 main.py"` |
| 실시간 로그 보기 | `tail -f ~/coin-monitor/log.txt` |
| config.yaml 수정 | `nano ~/coin-monitor/config.yaml` |
| config 저장 | `Ctrl+O` → 엔터 → `Ctrl+X` |
| 서버 접속 종료 | `exit` |

---

## 14. 바이낸스 API 차단 문제 해결 기록 (2026-04-20)

### 문제
Oracle Cloud 서버 IP가 바이낸스에 의해 지역 차단됨 → `451` 또는 `code: 0` 오류 발생

### 시도했던 방법과 실패 이유

| 방법 | 결과 | 실패 이유 |
|------|------|-----------|
| Cloudflare Workers 프록시 | ❌ 실패 | Cloudflare 출구 IP도 바이낸스가 차단 |
| Tor 프록시 | ❌ 실패 | Tor 출구 노드도 바이낸스가 차단 (CloudFront 403) |
| `data-api.binance.vision` | ✅ **성공** | 바이낸스 공식 마켓 데이터 전용 엔드포인트, 지역 제한 없음 |

### 최종 해결책
`exchanges.py`의 바이낸스 URL을 공식 데이터 엔드포인트로 변경:
```
api1.binance.com  →  data-api.binance.vision
```
이 엔드포인트는 인증 불필요한 공개 시세 데이터 전용이며, 현재 지역 제한이 없음.

> **참고**: 바이낸스는 `api.binance.com`, `api1~3.binance.com`, Cloudflare IP, Tor 출구 노드를 모두 차단하지만 `data-api.binance.vision`은 차단하지 않음 (2026-04-20 기준).
